...
TW_UINT16 TwainSession::setEnumerationCap(TW_CAPABILITY &cap, Napi::Object obj) {
    ...
    for (TW_UINT32 i = 0; i < numItems; ++i) {
        ...
        switch (itemType) {
            ...
            case TWTY_BOOL:
                ((TW_BOOL*)pEnum->ItemList)[i] = (TW_BOOL)item.As<Napi::Boolean>().Value() ? 1 : 0;
                break;
            default:
    ...
    // Send the capability setting command
...
TW_UINT16 TwainSession::setArrayCap(TW_CAPABILITY &cap, Napi::Object obj) {
    ...
    for (TW_UINT32 i = 0; i < numItems; ++i) {
        ...
        switch (itemType) {
            ...
            case TWTY_BOOL:
                ((TW_BOOL*)pArray->ItemList)[i] = (TW_BOOL)item.As<Napi::Boolean>().Value() ? 1 : 0;
                break;
            default:
                std::cerr << "Unsupported item type in loop" << std::endl;
                unlockMemory(cap.hContainer);
...
TW_UINT16 TwainSession::setOneValueCap(TW_CAPABILITY &cap, Napi::Object obj) {
    ...
    cap.ConType = TWON_ONEVALUE;
    std::cout << "start allocMemory:" << std::endl;
    // Extract itemType from the input object
    TW_UINT16 itemType = static_cast<TW_UINT16>(obj.Get("itemType").As<Napi::Number>().Uint32Value());
    cap.hContainer = allocMemory(sizeof(TW_ONEVALUE) - sizeof(TW_UINT32) + sizeof(TW_UINT32));
    if (cap.hContainer == NULL) {
        return TWRC_FAILURE;
    }
    std::cout << "start lockMemory:" << std::endl;
    ...
    switch (itemType) {
        ...
        case TWTY_STR128:
        case TWTY_STR255: {
            std::string strValue = obj.Get("value").As<Napi::String>().Utf8Value();
            strcpy(reinterpret_cast<char*>(&pOneValue->Item), strValue.c_str());
            break;
        }
        default:
            std::cerr << "Unsupported item type" << std::endl;
...
TW_UINT16 TwainSession::scan(TW_UINT32 mech, std::string path, Napi::Env env, Napi::Function callback,Napi::Number start) {
    if(state != 6) {
    ...
    switch (mech) {
        ...
        case TWSX_FILE: {
            ...
            if( rc == TWRC_SUCCESS){
                ...
                transferFile(pEnum->Item, path,env, callback,start);
            }
            break;
        }
        case TWSX_MEMORY: {
...
std::string TwainSession::convertImageFileFormatToExt(const TW_UINT16 value) {
    const char *text = NULL;
    switch (value) {
        case TWFF_PICT:
            text = ".pict";
